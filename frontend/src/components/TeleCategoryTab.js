...
export default function TeleCategoryTab({
  user,
  showToast,
  isSidebarCollapsed,
  permissions,
  ...
  const fetchTeleCategories = async () => {
    setLoading(true);
    try {
      const clientQuery = user?.clientid ? `?client_id=${user.clientid}` : '';
      const res = await fetch(`${API_URL}/api/tele-categories${clientQuery}`);
      if (!res.ok) throw new Error('Failed to fetch tele category data');
      const data = await res.json();
      setCategories(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error(err);
      if (showToast) showToast('Error loading tele categories', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = editingId
        ? `${API_URL}/api/tele-categories/${editingId}`
        : `${API_URL}/api/tele-categories`;
      ...
      if (showToast)
        showToast(
          editingId
            ? 'Tele category updated successfully'
      ...
      console.error(err);
      if (showToast) showToast(err.message || 'Error saving tele category', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this tele category?')) return;
    try {
      const res = await fetch(`${API_URL}/api/tele-categories/${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Delete failed');
      if (showToast) showToast('Tele category deleted successfully', 'success');
      fetchTeleCategories();
    } catch (err) {
      console.error(err);
      if (showToast) showToast('Error deleting tele category', 'error');
    }
  };

  const openModal = (categoryObj = null) => {
    if (categoryObj) {
      setEditingId(categoryObj.id);
      setCategoryName(categoryObj.category_name || '');
      setStatus(categoryObj.status || 'Active');
    } else {
      setEditingId(null);
      setCategoryName('');
      setStatus('Active');
      ...
      >
        <View style={styles.headerTitleGroup}>
          <View style={styles.iconSquareBadge}>
            <Ionicons name="pricetags-outline" size={22} color="#0284C7" />
          </View>
          <View style={styles.titleWrapper}>
            <Text style={styles.tabHeadingTitle}>Tele Category</Text>
            <Text style={styles.tabHeadingSubtitle}>
              Manage telecom categories for SIM cards and telecom services.
            </Text>
          </View>
        </View>
        ...
        {canCreate && (
          <TouchableOpacity style={styles.addBtn} onPress={() => openModal()} activeOpacity={0.8}>
            ...
            <Text style={styles.addBtnText}>Add Tele Category</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={styles.tableCard}>
        ...
        {loading ? (
          <View style={styles.tableLoaderContainer}>
            ...
            <Text style={styles.loaderText}>Loading tele categories...</Text>
          </View>
          ...
          <>
            <View style={styles.tableWrapper}>
              ...
              <View style={styles.tableHeader}>
                ...
                <Text style={[styles.thCell, { flex: 4.0 }]}>CATEGORY NAME</Text>
              ...
              {paginatedCategories.map((item, index) => {
                ...
                return (
                  <View
                    ...
                    style={[
                      styles.tableRow,
                      index === paginatedCategories.length - 1 && styles.lastTableRow,
                    ]}
                  >
          ...
          <View style={styles.emptyView}>
            ...
            <Text style={styles.emptyText}>
              {categories.length === 0
                ? 'No tele categories created yet.'
                : 'No matching tele categories found.'}
            </Text>
          </View>
        )}
      </View>
            ...
            <View style={styles.modalHeader}>
              <View style={styles.modalTitleWrapper}>
                ...
                <Text style={styles.modalTitle}>
                  {editingId ? 'Edit' : 'Add'} Tele Category
                </Text>
              </View>
            ...
            <View style={[styles.modalForm, { padding: 20 }]}>
              <View style={styles.formGroup}>
                <Text style={styles.inputLabel}>
                  Category Name(s) <Text style={{ color: '#EF4444' }}>*</Text>
                </Text>
                <TextInput
                  style={styles.textInput}
                  placeholder="e.g. Mobile SIM, Data Card, M2M, Voice Only, Roaming"
                  value={categoryName}
                ...
                {!editingId && (
                  <Text style={{ fontSize: 11, color: '#64748B', marginTop: 6 }}>
                    💡 Tip: Separate multiple category names with commas to add them in bulk.
                  </Text>
                )}
              </View>
