...
export default function VehicleDetailsTab({ user, showToast, isSidebarCollapsed, permissions, checkRowPermission }) {
  ...
  const fetchCompaniesForClient = async (clientId, overrideAction) => {
    ...
    try {
      ...
      const actionQuery = `&module_id=vehicle_details&action=${action}`;
  ...
  const fetchFormConfiguration = async (clientId, countryId, moduleId) => {
    ...
    try {
      ...
      if (!resolvedModuleId && modules && modules.length > 0) {
        const viModule = modules.find(m => m.module_name && m.module_name.toLowerCase().includes('vehicle details'));
        if (viModule) {
        ...
        {vehicleDetailsRecords.length === 0 && !searchQuery ? (
            ...
            <View style={{ flex: 1, backgroundColor: '#FFFFFF', borderRadius: 8, borderWidth: 1, borderColor: '#F1F5F9', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 3, elevation: 1, overflow: 'hidden' }}>
              ...
              <View style={{ flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#E2E8F0', paddingVertical: 14, paddingHorizontal: 20 }}>
                ...
                <Text style={{ flex: 1.5, fontSize: 11, fontWeight: '700', color: '#475569', textTransform: 'uppercase' }}>Vehicle Name</Text>
              ...
              <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={true}>
                {(() => {
                  const filtered = vehicleDetailsRecords.filter(r => {
                    ...
                    const pNo = r.plate_no || '';
                    const compName = r.company_name || '';
                    return String(r.id).includes(searchQuery) ||
                      (cName && cName.toLowerCase().includes(queryLower)) ||
                      (vName && vName.toLowerCase().includes(queryLower)) ||
                      (pNo && pNo.toLowerCase().includes(queryLower)) ||
                      (compName && compName.toLowerCase().includes(queryLower));
                  });
                  ...
                  return (
                    <View style={{ flex: 1 }}>
                      {paginated.map((record) => {
                        ...
                        const compName = record.company_name || (companyObj ? (companyObj.company_name || companyObj.name) : null) || parsedData['Company'] || 'N/A';

                        const stringValues = Object.values(parsedData).filter(v => typeof v === 'string' && v.trim());
                        const vehicleName = record.vehicle_name && record.vehicle_name !== 'N/A'
                          ? record.vehicle_name
                          : (stringValues[0] || 'N/A');

                        const plateNo = record.plate_no && record.plate_no !== 'N/A'
                          ? record.plate_no
                          : (stringValues[1] || 'N/A');
                        ...
                        return (
                          <View key={record.id} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 16, paddingHorizontal: 20, borderBottomWidth: 1, borderBottomColor: '#F1F5F9', backgroundColor: '#FFFFFF' }}>
                            ...
                            <View style={{ flex: 1.5, paddingRight: 10 }}>
                              <Text style={{ fontSize: 13, color: '#0F172A', fontWeight: '600' }} numberOfLines={1}>{vehicleName}</Text>
                            </View>
          ...
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                ...
                <Text style={styles.modalTitle}>{isViewOnly ? 'View Vehicle Details Record' : (editingRecord ? 'Edit Vehicle Details Record' : 'Add Vehicle Details')}</Text>
              </View>
                    ...
                    <View style={{ marginBottom: 20, zIndex: 30 }}>
                      ...
                      <SearchableDropdown
                        ...
                        onChange={(val) => {
                          ...
                          setSelectedCompany('');
                          fetchCompaniesForClient(val, isViewOnly ? 'view' : (editingRecord ? 'edit' : 'create'));
                        }}
