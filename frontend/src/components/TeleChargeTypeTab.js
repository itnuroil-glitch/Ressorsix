...
export default function TeleChargeTypeTab({
  user,
  showToast,
  isSidebarCollapsed,
  permissions,
  pageTitle = 'Tele Charge Type',
  pageSubtitle = 'Manage telecom charge types for usage and billing items.',
  addBtnText = 'Add Tele Charge Type',
  modalTitle = 'Tele Charge Type',
  ...
  useEffect(() => {
    fetchTeleChargeTypes();
  }, []);

  const fetchTeleChargeTypes = async () => {
    setLoading(true);
    try {
      const clientQuery = user?.clientid ? `?client_id=${user.clientid}` : '';
      const res = await fetch(`${API_URL}/api/tele-charge-types${clientQuery}`);
      if (!res.ok) throw new Error('Failed to fetch tele charge type data');
      const data = await res.json();
      setChargeTypes(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error(err);
      if (showToast) showToast('Error loading tele charge types', 'error');
    } finally {
      setLoading(false);
    }
  };

  const filteredChargeTypes = chargeTypes.filter((c) =>
    c.charge_type_name && c.charge_type_name.toLowerCase().includes(search.toLowerCase())
  );

  const totalPages = Math.max(1, Math.ceil(filteredChargeTypes.length / itemsPerPage));
  const currentPage = Math.min(page, totalPages);
  const paginatedChargeTypes = filteredChargeTypes.slice(
    (currentPage - 1) * itemsPerPage,
    currentPage * itemsPerPage
  );

  const handleSave = async () => {
    if (!chargeTypeName.trim()) {
      if (showToast) showToast('Charge type name is required', 'error');
      return;
    }
    ...
    try {
      const payload = {
        charge_type_name: chargeTypeName.trim(),
        status: status,
        client_id: user?.clientid || null,
      };

      const url = editingId
        ? `${API_URL}/api/tele-charge-types/${editingId}`
        : `${API_URL}/api/tele-charge-types`;
      ...
      if (showToast)
        showToast(
          editingId
            ? 'Tele charge type updated successfully'
      ...
      console.error(err);
      if (showToast) showToast(err.message || 'Error saving tele charge type', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this tele charge type?')) return;
    try {
      const res = await fetch(`${API_URL}/api/tele-charge-types/${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Delete failed');
      if (showToast) showToast('Tele charge type deleted successfully', 'success');
      fetchTeleChargeTypes();
    } catch (err) {
      console.error(err);
      if (showToast) showToast('Error deleting tele charge type', 'error');
    }
  };
  ...
  const openModal = (itemObj = null) => {
    if (itemObj) {
      ...
      setChargeTypeName(itemObj.charge_type_name || '');
      ...
      <View style={styles.tableCard}>
        ...
        <View style={styles.toolbarWrapper}>
          <View style={styles.searchBarWrapper}>
            ...
            <TextInput
              ...
              placeholder="Search charge type names..."
              value={search}
              onChangeText={(text) => {
                setSearch(text);
                setPage(1);
        ...
        {loading ? (
          <View style={styles.tableLoaderContainer}>
            ...
            <Text style={styles.loaderText}>Loading tele charge types...</Text>
          </View>
          ...
          <>
            <View style={styles.tableWrapper}>
              ...
              <View style={styles.tableHeader}>
                ...
                <Text style={[styles.thCell, { flex: 4.0 }]}>CHARGE TYPE NAME</Text>
              ...
              {/* Table Rows */}
              {paginatedChargeTypes.map((item, index) => {
                ...
                return (
                  <View
                    ...
                    style={[
                      styles.tableRow,
                      index === paginatedChargeTypes.length - 1 && styles.lastTableRow,
                    ]}
                  >
            ...
            <View style={styles.paginationRow}>
              <Text style={{ fontSize: 12, color: '#64748B' }}>
                ...
                to{' '}
                <Text style={{ fontWeight: '700', color: '#334155' }}>
                  {Math.min(currentPage * itemsPerPage, filteredChargeTypes.length)}
                </Text>{' '}
                of{' '}
                <Text style={{ fontWeight: '700', color: '#334155' }}>
                  {filteredChargeTypes.length}
                </Text>{' '}
                entries
              </Text>
          ...
          <View style={styles.emptyView}>
            ...
            <Text style={styles.emptyText}>
              {chargeTypes.length === 0
                ? 'No tele charge types created yet.'
                : 'No matching tele charge types found.'}
            </Text>
          </View>
        )}
      </View>
            ...
            <View style={[styles.modalForm, { padding: 20 }]}>
              <View style={styles.formGroup}>
                <Text style={styles.inputLabel}>
                  Charge Type Name(s) <Text style={{ color: '#EF4444' }}>*</Text>
                </Text>
                <TextInput
                  style={styles.textInput}
                  placeholder="e.g. Local Call, Roaming Data, Premium SMS, Late Fee"
                  value={chargeTypeName}
                  onChangeText={setChargeTypeName}
                  placeholderTextColor="#94A3B8"
                />
                {!editingId && (
                  <Text style={{ fontSize: 11, color: '#64748B', marginTop: 6 }}>
                    💡 Tip: Separate multiple charge type names with commas to add them in bulk.
                  </Text>
                )}
              </View>
