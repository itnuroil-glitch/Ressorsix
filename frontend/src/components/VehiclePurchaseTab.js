...
export default function VehiclePurchaseTab({ user, showToast, isSidebarCollapsed, permissions, checkRowPermission }) {
  ...
  const [vehiclePurchaseRecords, setVehiclePurchaseRecords] = useState([]);
  ...
  const fetchInitialData = async () => {
    try {
      const [clientsRes, countriesRes, modulesRes, recordsRes] = await Promise.all([
        ...
        fetch(`${API_URL}/api/vehicle-purchase${user && String(user.roleId) !== '1' && user.clientid ? `?clientid=${user.clientid}` : ''}`)
      ]);
      ...
      setVehiclePurchaseRecords(Array.isArray(recordsData) ? recordsData : []);
  ...
  const fetchCompaniesForClient = async (clientId, overrideAction) => {
    ...
    try {
      ...
      const actionQuery = `&module_id=vehicle_purchase&action=${action}`;
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      showToast('Vehicle purchase record deleted successfully', 'success');
      setDeleteModalVisible(false);
      setRecordToDelete(null);
      fetchInitialData(); // Refresh the list
    } catch (error) {
      console.error(error);
      showToast('Error deleting vehicle purchase record', 'error');
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = isEditing
        ? `${API_URL}/api/vehicle-purchase/${editingRecord.id}`
        : `${API_URL}/api/vehicle-purchase`;
      ...
      if (!res.ok) throw new Error(`Failed to ${isEditing ? 'update' : 'save'} vehicle purchase`);

      showToast(isEditing ? 'Vehicle purchase record updated successfully!' : 'Form submitted successfully!', 'success');
      setIsFormOpen(false);
      setEditingRecord(null);
      setFormData({});
      fetchInitialData();
      ...
      console.error(error);
      showToast('Error saving vehicle purchase', 'error');
    } finally {
      setSaving(false);
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.mainContent}>
        {false ? (
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 24, minHeight: 400 }}>
            <View style={{
              ...
              <Text style={{ fontSize: 24, fontWeight: '800', color: COLORS.textPrimary, marginBottom: 12, textAlign: 'center' }}>
                Vehicle Purchase Entry
              </Text>
              <Text style={{ fontSize: 14, color: COLORS.textSecondary, textAlign: 'center', marginBottom: 32, lineHeight: 22, maxWidth: 440 }}>
                Please register the vehicle purchase transaction and details. All records are processed securely.
              </Text>
              <TouchableOpacity
                style={{
          ...
          vehiclePurchaseRecords.length === 0 && !searchQuery ? (
            <View style={styles.emptyState}>
              <Ionicons name="document-text-outline" size={48} color={COLORS.textMuted} />
              <Text style={styles.emptyStateText}>No vehicle purchase records found.</Text>
            ...
            <View style={{ flex: 1, backgroundColor: '#FFFFFF', borderRadius: 8, borderWidth: 1, borderColor: '#F1F5F9', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 3, elevation: 1, overflow: 'hidden' }}>
              ...
              <View style={{ flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#E2E8F0', paddingVertical: 14, paddingHorizontal: 20 }}>
                ...
                <Text style={{ flex: 1.3, fontSize: 11, fontWeight: '700', color: '#475569', textTransform: 'uppercase' }}>Plate Number</Text>
                <Text style={{ flex: 1.3, fontSize: 11, fontWeight: '700', color: '#475569', textTransform: 'uppercase' }}>Purchase Date</Text>
                <Text style={{ flex: 1.4, fontSize: 11, fontWeight: '700', color: '#475569', textTransform: 'uppercase' }}>Supplier</Text>
                <Text style={{ flex: 1.3, fontSize: 11, fontWeight: '700', color: '#475569', textTransform: 'uppercase' }}>Purchase Price</Text>
              ...
              <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={true}>
                {(() => {
                  const filtered = vehiclePurchaseRecords.filter(r => {
                    ...
                    const vehName = r.vehicle_name || r.vehicle_display_name || '';
                    const plateNo = r.plate_no || '';
                    return String(r.id).includes(searchQuery) ||
                      ...
                      (plateNo && plateNo.toLowerCase().includes(queryLower));
                  });
                  ...
                  return (
                    <View style={{ flex: 1 }}>
                      {paginated.map((record) => {
                        ...
                        let vehicleName = record.vehicle_name && record.vehicle_name !== 'N/A' 
                          ? record.vehicle_name 
                          : (record.vehicle_display_name ? record.vehicle_display_name.split(' - ')[0] : 'N/A');

                        let plateNo = record.plate_no && record.plate_no !== 'N/A' 
                          ? record.plate_no 
                          : (record.vehicle_display_name && record.vehicle_display_name.includes(' - ') ? record.vehicle_display_name.split(' - ')[1] : 'N/A');

                        if (vehicleName.includes(' - ')) {
                          const parts = vehicleName.split(' - ');
                          vehicleName = parts[0].trim();
                          if (plateNo === 'N/A' || /^\d{4}-\d{2}-\d{2}/.test(plateNo)) {
                            plateNo = parts[1].trim();
                          }
                        }

                        if (/^\d{4}-\d{2}-\d{2}/.test(plateNo) && record.vehicle_display_name && record.vehicle_display_name.includes(' - ')) {
                          plateNo = record.vehicle_display_name.split(' - ')[1].trim();
                        }

                        const companyObj = companies.find(c => String(c.id) === String(record.company_id || record.companyid));
                        const compName = record.company_name || (companyObj ? (companyObj.company_name || companyObj.name) : null) || 'N/A';
                        ...
                        return (
                          <View key={record.id} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 16, paddingHorizontal: 20, borderBottomWidth: 1, borderBottomColor: '#F1F5F9', backgroundColor: '#FFFFFF' }}>
                            ...
                            <View style={{ flex: 1.3, paddingRight: 10 }}>
                              <Text style={{ fontSize: 13, color: '#0F172A', fontWeight: '600' }} numberOfLines={1}>{plateNo}</Text>
                            </View>

                            <View style={{ flex: 1.3, paddingRight: 10 }}>
                              <Text style={{ fontSize: 13, color: '#475569', fontWeight: '500' }} numberOfLines={1}>{formatDate(record.purchase_date)}</Text>
                            </View>

                            <View style={{ flex: 1.4, paddingRight: 10 }}>
                              <Text style={{ fontSize: 13, color: '#475569', fontWeight: '500' }} numberOfLines={1}>{record.supplier || 'N/A'}</Text>
                            </View>

                            <View style={{ flex: 1.3, paddingRight: 10 }}>
                              <Text style={{ fontSize: 13, color: '#475569', fontWeight: '500' }} numberOfLines={1}>{record.purchase_price || 'N/A'}</Text>
                            </View>

                            <View style={{ flex: 1.5, paddingRight: 10 }}>
                              <Text style={{ fontSize: 13, color: '#0F172A', fontWeight: '600' }} numberOfLines={1}>{compName}</Text>
                            </View>
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center', alignItems: 'center' }}>
          <View style={{ backgroundColor: '#FFFFFF', borderRadius: 12, width: 450, padding: 24, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 12, elevation: 5 }}>
            ...
            <Text style={{ fontSize: 14, color: '#64748B', marginBottom: 24, lineHeight: 20 }}>
              This will permanently delete the selected vehicle purchase record. This action cannot be undone and will be completely removed from the database.
            </Text>
