...
export default function CompanyDefCurrencyTab({
  user,
  showToast,
  isSidebarCollapsed,
  permissions,
  ...
  useEffect(() => {
    fetchCompanyDefCurrencies();
  }, []);

  const fetchCompanyDefCurrencies = async () => {
    setLoading(true);
    try {
      const clientQuery = user?.clientid ? `?client_id=${user.clientid}` : '';
      const res = await fetch(`${API_URL}/api/company-def-currency${clientQuery}`);
      if (!res.ok) throw new Error('Failed to fetch company default currencies');
      const data = await res.json();
      setCurrencies(Array.isArray(data) ? data : []);
      if (onRefreshOptions) onRefreshOptions();
    } catch (err) {
      console.error(err);
      if (showToast) showToast('Error loading company default currencies', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredCurrencies = currencies.filter((c) =>
    (c.currency_name && c.currency_name.toLowerCase().includes(search.toLowerCase())) ||
    (c.currency_code && c.currency_code.toLowerCase().includes(search.toLowerCase()))
  );

  const totalPages = Math.max(1, Math.ceil(filteredCurrencies.length / itemsPerPage));
  ...
  const handleSave = async () => {
    if (!currencyName.trim()) {
      if (showToast) showToast('Currency name is required', 'error');
      return;
    }
    ...
    try {
      const payload = {
        currency_code: currencyCode.trim() || currencyName.trim().substring(0, 5).toUpperCase(),
      ...
      const url = editingId
        ? `${API_URL}/api/company-def-currency/${editingId}`
        : `${API_URL}/api/company-def-currency`;
      ...
      console.error(err);
      if (showToast) showToast(err.message || 'Error saving default currency', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this default currency?')) return;
    try {
      const res = await fetch(`${API_URL}/api/company-def-currency/${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Delete failed');
      if (showToast) showToast('Default currency deleted successfully', 'success');
      fetchCompanyDefCurrencies();
    } catch (err) {
      console.error(err);
      if (showToast) showToast('Error deleting default currency', 'error');
    }
  };

  const openModal = (currObj = null) => {
    if (currObj) {
      setEditingId(currObj.id);
      setCurrencyCode(currObj.currency_code || '');
      setCurrencyName(currObj.currency_name || '');
      setStatus(currObj.status || 'Active');
    } else {
      setEditingId(null);
      setCurrencyCode('');
      setCurrencyName('');
      ...
      >
        <View style={styles.headerTitleGroup}>
          ...
          <View style={styles.titleWrapper}>
            ...
            <Text style={styles.tabHeadingSubtitle}>
              Manage default currencies and monetary symbols available for company registration.
            </Text>
          </View>
        </View>
        ...
        {canCreate && (
          <TouchableOpacity style={styles.addBtn} onPress={() => openModal()} activeOpacity={0.8}>
            ...
            <Text style={styles.addBtnText}>Add Default Currency</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={styles.tableCard}>
        ...
        {loading ? (
          <View style={styles.tableLoaderContainer}>
            ...
            <Text style={styles.loaderText}>Loading default currencies...</Text>
          </View>
          ...
          <>
            <View style={styles.tableWrapper}>
              ...
              <View style={styles.tableHeader}>
                ...
                <Text style={[styles.thCell, { flex: 2.0 }]}>CODE</Text>
                <Text style={[styles.thCell, { flex: 4.0 }]}>CURRENCY NAME</Text>
              ...
              {paginatedCurrencies.map((item, index) => {
                ...
                return (
                  <View
                    ...
                    style={[
                      styles.tableRow,
                      index === paginatedCurrencies.length - 1 && styles.lastTableRow,
                    ]}
                  >
                    ...
                    <View style={{ flex: 2.0, flexDirection: 'row', alignItems: 'center' }}>
                      <View style={{ backgroundColor: '#F1F5F9', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, borderWidth: 1, borderColor: '#CBD5E1' }}>
                        <Text style={{ fontSize: 12, fontWeight: '800', color: '#1E293B', fontFamily: 'monospace' }}>
                          {item.currency_code}
                        </Text>
                      </View>
                    </View>
            ...
            <View style={styles.paginationRow}>
              <Text style={{ fontSize: 12, color: '#64748B' }}>
                ...
                to{' '}
                <Text style={{ fontWeight: '700', color: '#334155' }}>
                  {Math.min(currentPage * itemsPerPage, filteredCurrencies.length)}
                </Text>{' '}
                of{' '}
                <Text style={{ fontWeight: '700', color: '#334155' }}>
          ...
          <View style={styles.emptyView}>
            ...
            <Text style={styles.emptyText}>
              {currencies.length === 0
                ? 'No default currencies created yet.'
                : 'No matching default currencies found.'}
            </Text>
          </View>
        )}
      </View>
            ...
            <View style={styles.modalHeader}>
              <View style={styles.modalTitleWrapper}>
                ...
                <Text style={styles.modalTitle}>
                  {editingId ? 'Edit' : 'Add'} Default Currency
                </Text>
              </View>
            ...
            <View style={[styles.modalForm, { padding: 20 }]}>
              <View style={styles.formGroup}>
                ...
                <TextInput
                  ...
                  placeholder="e.g. AED, USD, EUR, GBP"
                  value={currencyCode}
                  onChangeText={(val) => setCurrencyCode(val.toUpperCase())}
                  placeholderTextColor="#94A3B8"
                  maxLength={10}
                />
              </View>
              ...
              <View style={styles.formGroup}>
                <Text style={styles.inputLabel}>
                  Currency Name <Text style={{ color: '#EF4444' }}>*</Text>
                </Text>
                <TextInput
                  style={styles.textInput}
                  placeholder="e.g. UAE Dirham (AED), US Dollar (USD)"
                  value={currencyName}
