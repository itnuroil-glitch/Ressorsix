...
export default function CompanyLicenseAuthTab({
  user,
  showToast,
  isSidebarCollapsed,
  permissions,
  ...
  const [licenseAuths, setLicenseAuths] = useState([]);
  ...
  useEffect(() => {
    fetchCompanyLicenseAuths();
  }, []);

  const fetchCompanyLicenseAuths = async () => {
    setLoading(true);
    try {
      const clientQuery = user?.clientid ? `?client_id=${user.clientid}` : '';
      const res = await fetch(`${API_URL}/api/company-license-auth${clientQuery}`);
      if (!res.ok) throw new Error('Failed to fetch company licensing authority data');
      const data = await res.json();
      setLicenseAuths(Array.isArray(data) ? data : []);
      if (onRefreshOptions) onRefreshOptions();
    } catch (err) {
      console.error(err);
      if (showToast) showToast('Error loading company licensing authorities', 'error');
    } finally {
      setLoading(false);
    }
  };

  const filteredAuths = licenseAuths.filter((a) =>
    a.authority_name && a.authority_name.toLowerCase().includes(search.toLowerCase())
  );

  const totalPages = Math.max(1, Math.ceil(filteredAuths.length / itemsPerPage));
  const currentPage = Math.min(page, totalPages);
  const paginatedAuths = filteredAuths.slice(
  ...
  const handleSave = async () => {
    if (!authorityName.trim()) {
      if (showToast) showToast('Authority name is required', 'error');
      return;
    }
    ...
    try {
      ...
      const url = editingId
        ? `${API_URL}/api/company-license-auth/${editingId}`
        : `${API_URL}/api/company-license-auth`;
      ...
      console.error(err);
      if (showToast) showToast(err.message || 'Error saving licensing authority', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this licensing authority?')) return;
    try {
      const res = await fetch(`${API_URL}/api/company-license-auth/${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Delete failed');
      if (showToast) showToast('Licensing authority deleted successfully', 'success');
      fetchCompanyLicenseAuths();
    } catch (err) {
      console.error(err);
      if (showToast) showToast('Error deleting licensing authority', 'error');
    }
  };

  const openModal = (authObj = null) => {
    if (authObj) {
      setEditingId(authObj.id);
      setAuthorityName(authObj.authority_name || '');
      setStatus(authObj.status || 'Active');
    } else {
      setEditingId(null);
      setAuthorityName('');
      setStatus('Active');
      ...
      >
        <View style={styles.headerTitleGroup}>
          ...
          <View style={styles.titleWrapper}>
            <Text style={styles.tabHeadingTitle}>Company Licensing Authority</Text>
            <Text style={styles.tabHeadingSubtitle}>
              Manage trade licensing authorities and regulatory bodies for company registration.
            </Text>
          </View>
        </View>
        ...
        {canCreate && (
          <TouchableOpacity style={styles.addBtn} onPress={() => openModal()} activeOpacity={0.8}>
            ...
            <Text style={styles.addBtnText}>Add Licensing Authority</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={styles.tableCard}>
        ...
        <View style={styles.toolbarWrapper}>
          <View style={styles.searchBarWrapper}>
            ...
            <TextInput
              ...
              placeholder="Search authority names..."
              value={search}
              onChangeText={(text) => {
                setSearch(text);
                setPage(1);
        ...
        {loading ? (
          <View style={styles.tableLoaderContainer}>
            ...
            <Text style={styles.loaderText}>Loading company licensing authorities...</Text>
          </View>
          ...
          <>
            <View style={styles.tableWrapper}>
              ...
              <View style={styles.tableHeader}>
                ...
                <Text style={[styles.thCell, { flex: 4.0 }]}>AUTHORITY NAME</Text>
              ...
              {paginatedAuths.map((item, index) => {
                ...
                return (
                  <View
                    ...
                    style={[
                      styles.tableRow,
                      index === paginatedAuths.length - 1 && styles.lastTableRow,
                    ]}
                  >
            ...
            <View style={styles.paginationRow}>
              <Text style={{ fontSize: 12, color: '#64748B' }}>
                ...
                to{' '}
                <Text style={{ fontWeight: '700', color: '#334155' }}>
                  {Math.min(currentPage * itemsPerPage, filteredAuths.length)}
                </Text>{' '}
                of{' '}
                <Text style={{ fontWeight: '700', color: '#334155' }}>
          ...
          <View style={styles.emptyView}>
            ...
            <Text style={styles.emptyText}>
              {licenseAuths.length === 0
                ? 'No licensing authorities created yet.'
                : 'No matching licensing authorities found.'}
            </Text>
          </View>
        )}
      </View>
            ...
            <View style={styles.modalHeader}>
              <View style={styles.modalTitleWrapper}>
                ...
                <Text style={styles.modalTitle}>
                  {editingId ? 'Edit' : 'Add'} Licensing Authority
                </Text>
              </View>
            ...
            <View style={[styles.modalForm, { padding: 20 }]}>
              <View style={styles.formGroup}>
                <Text style={styles.inputLabel}>
                  Authority Name(s) <Text style={{ color: '#EF4444' }}>*</Text>
                </Text>
                <TextInput
                  style={styles.textInput}
                  placeholder="e.g. DET Dubai, DMCC, JAFZA, ADDED"
                  value={authorityName}
                ...
                {!editingId && (
                  <Text style={{ fontSize: 11, color: '#64748B', marginTop: 6 }}>
                    💡 Tip: Separate multiple authority names with commas to add them in bulk.
                  </Text>
                )}
              </View>
              ...
              >
                {saving ? (
                  ...
                  <Text style={styles.saveBtnText}>
                    {editingId ? 'Save Changes' : 'Create Authority'}
                  </Text>
                )}
              </TouchableOpacity>
            </View>
