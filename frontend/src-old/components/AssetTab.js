...
export default function AssetTab({ user, showToast, renderTableToolbar, renderTablePagination, isSidebarCollapsed }) {
  ...
  const fetchCompaniesForClient = async (clientId) => {
    ...
    try {
      ...
      const actionQuery = `&module_id=asset_details&action=view`;
      const res = await fetch(`${API_URL}/api/companies/client/${clientId}${emailParam}${actionQuery}`);
      if (res.ok) {
        let data = await res.json();
        if (isEmployee) {
          const userCompanyId = String(user.companyid || user.company_id || '');
          const associatedIds = Array.isArray(user.associatedCompanyIds)
            ? user.associatedCompanyIds.map(String)
            : (user.associatedCompanyIds ? String(user.associatedCompanyIds).split(',').map(s => s.trim()) : []);
          
          data = (data || []).filter(c => {
            const cidStr = String(c.id);
            return cidStr === userCompanyId || associatedIds.includes(cidStr);
          });
        }
        setCompanies(data || []);
        return data || [];
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const companyVal = user?.company_id || user?.companyid;
      if (companyVal) {
        setSelectedCompany(String(companyVal));
      }
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        status: status,
        company_id: selectedCompany ? parseInt(selectedCompany, 10) : null,
        clientid: selectedClient ? parseInt(selectedClient, 10) : null
      };
  ...
  const openModal = async (asset = null) => {
    if (asset) {
      ...
      const clientVal = asset.clientid || user?.client_id || user?.clientid;
      if (clientVal) {
        setSelectedClient(String(clientVal));
        await fetchCompaniesForClient(String(clientVal));
      }
      setSelectedCompany(asset.company_id ? String(asset.company_id) : '');
    } else {
      setEditingId(null);
      setAssetTagNumber('');
      setCategoryId('');
      ...
      setSelectedCompany(user?.company_id || user?.companyid ? String(user?.company_id || user?.companyid) : '');
    }
    setIsModalOpen(true);
  };
              ...
              {(!user || String(user.roleId) === '1') && (
                <View style={styles.formRow}>
                  <View style={[styles.formGroup, { flex: 1 }]}>
                    <Text style={styles.inputLabel}>Client <Text style={{ color: 'red' }}>*</Text></Text>
                    <div style={{ position: 'relative' }}>
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setSelectedCompany('');
                          fetchCompaniesForClient(e.target.value);
                        }}
