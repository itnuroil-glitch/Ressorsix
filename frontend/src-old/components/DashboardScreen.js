...
export default function DashboardScreen({ user, onSignOut }) {
  ...
  const fetchUserPermissions = () => {
    ...
    const compIds = (user.associatedCompanyIds && user.associatedCompanyIds.length > 0)
      ...
      : (user.companyid || '');
    let url = `${API_URL}/api/roles/${user.roleId}/permissions?company_id=${compIds}`;
    if (user.clientid) {
  ...
  const handleSavePermissions = () => {
    ...
    if (selectedPermissionCompanyId === 'all' || !selectedPermissionCompanyId) {
      targetCompanyIds = ['all'];
    } else {
      targetCompanyIds = [selectedPermissionCompanyId];
    }
  ...
  const startEditRole = (item) => {
    ...
    const clientIds = Array.isArray(item.clientids) ? item.clientids.map(String) : (item.clientid ? [String(item.clientid)] : []);
    ...
    fetch(url, {
      ...
      .then((data) => {
        ...
        if (rolePermissions && rolePermissions.length > 0) {
          const targetCompIds = (modalPermissionCompanyId && modalPermissionCompanyId !== 'all')
            ? [Number(modalPermissionCompanyId)]
  ...
  const hasViewPermission = (moduleId) => {
    ...
    if (userCompanyPermissions && userCompanyPermissions.length > 0) {
      const activeCompanyId = user?.companyid || (user?.associatedCompanyIds && user.associatedCompanyIds[0]) || null;
      if (activeCompanyId) {
        const compPerms = userCompanyPermissions.filter(
          p => p.module_id === moduleId && String(p.company_id) === String(activeCompanyId)
        );
    ...
    if (companyId) {
      ...
      if (compPerms.length > 0) {
        return compPerms.some(p => {
          if (p.full_control) return true;
          if (action === 'view' && p.can_view) return true;
          if (action === 'create' && p.can_create) return true;
          if (action === 'edit' && p.can_edit) return true;
          if (action === 'delete' && p.can_delete) return true;
          return false;
        });
      }
    }
