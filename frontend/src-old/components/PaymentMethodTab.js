...
export default function PaymentMethodTab({ user, showToast, renderTableToolbar, renderTablePagination, isSidebarCollapsed, permissions }) {
  ...
  const fetchPaymentMethods = async () => {
    ...
    try {
      ...
      console.error(err);
      showToast('Error loading asset paymentMethods', 'error');
    } finally {
      setLoading(false);
    }
  };

  const filteredPaymentMethods = paymentMethods.filter(b => 
    b.payment_name && b.payment_name.toLowerCase().includes(search.toLowerCase())
  );

  const displayPage = Math.min(page, Math.max(1, Math.ceil(filteredPaymentMethods.length / ITEMS_PER_PAGE)));
  const paginatedPaymentMethods = filteredPaymentMethods.slice((displayPage - 1) * ITEMS_PER_PAGE, displayPage * ITEMS_PER_PAGE);

  const handleSave = async () => {
    if (!paymentMethodName.trim()) {
      showToast('PaymentMethod name is required', 'error');
      return;
    }
    ...
    try {
      ...
      const url = editingId ? `${API_URL}/api/payment-methods/${editingId}` : `${API_URL}/api/payment-methods`;
      ...
      showToast(editingId ? 'PaymentMethod updated successfully' : 'PaymentMethod created successfully', 'success');
      setIsModalOpen(false);
      fetchPaymentMethods();
    } catch (err) {
      console.error(err);
      showToast(err.message || 'Error saving paymentMethod', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this paymentMethod?')) return;
    try {
      const res = await fetch(`${API_URL}/api/payment-methods/${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Delete failed');
      showToast('PaymentMethod deleted successfully', 'success');
      fetchPaymentMethods();
    } catch (err) {
      console.error(err);
      showToast('Error deleting paymentMethod', 'error');
    }
  };
  ...
  const openModal = (paymentMethod = null) => {
    if (paymentMethod) {
      ...
      setPaymentMethodName(paymentMethod.payment_name || '');
      setStatus(paymentMethod.status || 'Active');
    } else {
      setEditingId(null);
  ...
  return (
    <ScrollView style={styles.tabContent} keyboardShouldPersistTaps="handled">
      ...
      <View style={[styles.headerContainer, !isLargeScreen && { flexDirection: 'column', alignItems: 'flex-start', gap: 16 }]}>
        <View style={{ flexDirection: 'row', alignItems: 'center', flex: isLargeScreen ? 1 : undefined, width: isLargeScreen ? 'auto' : '100%', gap: 16 }}>
          ...
          <View style={styles.titleWrapper}>
            <Text style={[styles.tabHeadingTitle, { marginBottom: 2 }]}>Payment Methods</Text>
            <Text style={styles.tabHeadingSubtitle}>
              Manage manufacturer paymentMethods for your IT equipment.
            </Text>
          </View>
        </View>
        ...
        {canCreate && (
          <TouchableOpacity style={styles.addBtn} onPress={() => openModal()} activeOpacity={0.8}>
            ...
            <Text style={styles.addBtnText}>Add PaymentMethod</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={[styles.tableCard, { marginTop: SPACING.md }]}>
        {renderTableToolbar(search, setSearch, setPage, 'Search paymentMethods...')}

        {loading ? (
          <View style={styles.tableLoaderContainer}>
            <ActivityIndicator size="large" color={COLORS.primary} />
            <Text style={styles.loaderText}>Loading paymentMethods...</Text>
          </View>
          ...
          <>
            <ScrollView horizontal={true} showsHorizontalScrollIndicator={true} style={{ width: '100%' }} contentContainerStyle={{ minWidth: '100%' }}>
              <View style={[styles.tableWrapper, { minWidth: 600 }]}>
                <View style={{ paddingBottom: 10 }}>
                  ...
                  {/* Table Data Rows */}
                  {paginatedPaymentMethods.map((item, index) => {
                    const isInactive = item.status !== 'Active';

                    return (
                      <View key={item.id} style={[styles.tableRow, index === paginatedPaymentMethods.length - 1 && styles.lastTableRow]}>
            ...
            {renderTablePagination(filteredPaymentMethods.length, page, setPage)}
          </>
        ) : (
          <View style={styles.emptyView}>
            <Ionicons name="pricetag-outline" size={44} color={COLORS.textMuted} />
            <Text style={styles.emptyText}>{paymentMethods.length === 0 ? "No paymentMethods created yet." : "No matching paymentMethods found."}</Text>
          </View>
        )}
      </View>
          ...
          <View style={[styles.modalCard, { maxWidth: 500 }]}>
            ...
            <View style={[styles.modalForm, { padding: 20 }]}>
              ...
              <View style={styles.formGroup}>
                <Text style={styles.inputLabel}>PaymentMethod Name <Text style={{color: 'red'}}>*</Text></Text>
                <TextInput
            ...
            <View style={styles.modalFooter}>
              ...
              >
                {saving ? (
                  ...
                  <Text style={styles.saveBtnText}>{editingId ? 'Save Changes' : 'Create PaymentMethod'}</Text>
                )}
              </TouchableOpacity>
            </View>
          </View>
