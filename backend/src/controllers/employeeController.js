...
exports.getImportTemplateOptions = async (req, res) => {
  try {
    ...
    const isSuperAdmin = String(currentUser.roleId) === '1';
    const clientId = currentUser.clientid;

    // 1. Fetch active companies
    let companyQuery = `
      SELECT DISTINCT c.id, c.company_name, c.clientid
      FROM company c
    ...
    if (!isSuperAdmin) {
      if (!clientId) {
        return res.status(200).json({ roles: [], departments: [], companies: [] });
      }
      companyParams.push(clientId);
      companyQuery += ` AND (c.clientid = $1 OR u.clientid = $1)`;
    }
    companyQuery += ` ORDER BY c.company_name ASC`;

    const companiesRes = await db.query(companyQuery, companyParams);
    ...
    if (!isSuperAdmin && clientId) {
      ...
      roleQuery += ` AND (clientid = $1 OR clientid IS NULL)`;
    }
    roleQuery += ` ORDER BY role ASC`;
    const rolesRes = await db.query(roleQuery, roleParams);
    ...
    console.error('Error fetching import template options:', error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
};
