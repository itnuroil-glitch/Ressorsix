  ...
  try {
    ...
    if (companyId && companyId !== 'null' && companyId !== '') {
      ...
      const checkCompPerm = await db.query(
        'SELECT 1 FROM role_permission WHERE role_id = ANY($1) AND company_id = ANY(string_to_array($2, \',\')::int[]) LIMIT 1',
        [roleIds, String(companyId)]
      );
      ...
      if (checkCompPerm.rows.length > 0) {
        queryText = `
          SELECT 
            ...
            m.parent_id,
            0 AS permission_id,
            bool_and(COALESCE(rp.can_view, false)) AS can_view,
            bool_and(COALESCE(rp.can_create, false)) AS can_create,
            bool_and(COALESCE(rp.can_edit, false)) AS can_edit,
            bool_and(COALESCE(rp.can_delete, false)) AS can_delete,
            bool_and(COALESCE(rp.all_record_view, false)) AS all_record_view,
            bool_and(COALESCE(rp.full_control, false)) AS full_control
          FROM module m
          LEFT JOIN role_permission rp ON rp.module_id = m.id AND rp.role_id = ANY($1) AND rp.company_id = ANY(string_to_array($2, ',')::int[])
        ...
        queryText = `
          SELECT 
            ...
            bool_and(COALESCE(rp.full_control, false)) AS full_control
          FROM module m
          LEFT JOIN role_permission rp ON rp.module_id = m.id AND rp.role_id = ANY($1) AND rp.company_id IS NULL
          WHERE m.status = 'active' AND m.is_deleted = false
          GROUP BY m.id, m.module_name, m.parent_id
          ORDER BY m.parent_id ASC NULLS FIRST, m.id ASC
        `;
    ...
    const companyPermRes = await db.query(
      `SELECT 
       ...
       GROUP BY rp.module_id, rp.company_id`,
      [roleIds]
    );
...
exports.saveRolePermissions = async (req, res) => {
  ...
  try {
    ...
    const roleCheck = await client.query('SELECT id, clientid, clientids FROM role WHERE id = $1 AND is_deleted = false', [roleId]);
    if (roleCheck.rows.length === 0) {
      return res.status(404).json({ message: 'Role not found or has been deleted.' });
    }
    const roleObj = roleCheck.rows[0];

    // Get all company IDs associated with this role via clientid or clientids
    let allAssociatedCompanyIds = [];
    if (Array.isArray(roleObj.clientids) && roleObj.clientids.length > 0) {
      allAssociatedCompanyIds.push(...roleObj.clientids);
    }
    if (roleObj.clientid) {
