    ...
    if (email && email.trim() !== '') {
      ...
      if (empRes.rows.length > 0) {
        ...
        if (!isSuperAdminOrClientAdmin) {
          ...
          // Fetch assigned companies from role's clientids (multi roles supported via ANY)
          const roleRes = await pool.query(
            'SELECT clientids FROM role WHERE id = ANY(string_to_array($1, \',\')::int[]) AND is_deleted = false',
            [roleId]
          );
          const roleCompanyIds = [];
          roleRes.rows.forEach(r => {
            if (Array.isArray(r.clientids)) {
              r.clientids.forEach(id => roleCompanyIds.push(id));
            }
          });
